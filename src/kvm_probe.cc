...
KVM_probe& KVM_probe::operator=(KVM_probe&& other) noexcept {
  if (this != &other) {
    if (m_kvm_fd >= 0) close(m_kvm_fd);
    m_kvm_fd = std::exchange(other.m_kvm_fd, -1);
  }
  return *this;
}

std::expected<void, std::error_code> KVM_probe::initialize() {
  assert(m_kvm_fd == -1);
...
std::expected<int, std::error_code> KVM_probe::get_api_version() const {
...
bool KVM_probe::check_extension(int extension) const {
  return ioctl(m_kvm_fd, KVM_CHECK_EXTENSION, extension) > 0;
}

std::expected<size_t, std::error_code> KVM_probe::get_vcpu_mmap_size() const {
...
void KVM_probe::print_capabilities() const {
  ...
  for (const auto& ext : extensions) {
    ...
    println("{:<35} {:<8} {}", ext.m_name, supported ? "[YES]" : "[NO]", ext.m_description);
  }
  ...
  if (int hard_max_vcpus = ioctl(m_kvm_fd, KVM_CHECK_EXTENSION, KVM_CAP_MAX_VCPUS);
      hard_max_vcpus > 0) {
    println("Hard limit max VCPUs: {}", hard_max_vcpus);
  }

  if (int max_memslots = ioctl(m_kvm_fd, KVM_CHECK_EXTENSION, KVM_CAP_NR_MEMSLOTS);
      max_memslots > 0) {
